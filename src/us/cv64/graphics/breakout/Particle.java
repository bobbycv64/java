...
public class Particle {
	 ...
	 * loc = new Point2D.Double(x, y); // vel = new Point2D.Double(xvel, yvel);
	 * magnitude = ch; active = false; }
	 */
	...
	public void setLoc(double newX, double newY) {
		loc = new Point2D.Double(newX, newY);
	}
	...
	public void setMag(double newCh) {
		magnitude = newCh;
	}
	...
	public void update() {
		loc = new Point2D.Double(loc.x + speed * Math.cos(angle), loc.y + speed * Math.sin(angle));
	}

	public void bounce(double dev) {
		angle = angle + Math.random() * (dev) - dev / 2;
	}
}
